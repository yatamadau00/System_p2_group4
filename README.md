...
4. **残す** — フローティングボタンから1ページで完結。**ことづては「いまいる場所」にだけ残せます**（不特定の場所に置けてしまう無法を防ぐ）。本文に加えて **写真・映像・声を自由に重ねて** 添えられ、**声はその場で録音**できます。

ピンは縦型のしずく型で、伝書鳩のモチーフ＋中身の種類を示す小さな種別チップを備えます。距離ラベルは本体の上に余白をとって配置し、ピンと重なりません。
...
└─ components/
   ...
   ├─ ComposeFlow.tsx        残す（1ページ・今いる場所・複数メディア）
   ├─ AudioRecorder.tsx      その場録音のインラインUI
   ├─ ListSheet.tsx          一覧（みんな/わたし、削除）
   ├─ MediaView.tsx          開封後のメディア描画（1点ぶん）
