...
**既存DBを残したままグループ機能を追加したい場合は、[`supabase/groups.sql`](supabase/groups.sql) を実行してください**（データは消えません・何度実行してもOK）。
`kotozute` に `group_id` 列を追加し、`groups` / `group_members` テーブル（グループとそのメンバーをユーザーに紐づける）を作成します。
